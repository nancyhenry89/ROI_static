...
function registerResult(result,status) {
    ...
    if (result=='1') {
        ...
        if (result=='-4') {
            ...
            $('.signup-title').removeClass('head2')
            $('.signup-title').addClass('head1');
        }
...
function doRegister(api, manager_id, form, statusDOM ) {
    ...
    return new Promise( function (resolve,reject) {
        fetch("./register.php?api="+api, { method: "POST", body: form, credentials: "same-origin" })
        .then( function(response) { 
...
document.querySelector('form.boutiqueSignUp').addEventListener(
    ...
    function(evt) {
        ...
        var msg = showMessage();
        msg.innerHTML = 'Registering you to ROI Boutique... <span id="roi-signup-status" class="signup-status inline-loading"></span><br>';
...
function formCheckboxArray(name){
    ...
    $('input[name="'+name+'"]').each(function(){
        ...
        $('.placeholder[name="'+name+'"]').text(arr);
...
$('document').ready(function(){
    $('#cont').click(function(){
        $('input[type="password"],input[type="email"]').each(function(){
            ...
            $('.signup-title').addClass('head2');
            $('.signup-title').removeClass('head1');
            $('.step').removeClass('active');
