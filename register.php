...
$apis = array(
	...
	/*
	'mediaroi'=> array(
		'url'=>"https://mediaroi.api.hasoffers.com/Apiv3/json?",
		'token'=>"NET4Kk4jW7sKjrVhia3oV1yA5IvPvx"
	),
	*/
);
...
function checkEmail($email) {
	...
	global $apis;
	foreach ($apis as $a) {
		$api = new AffiliateApi ($a['url'], $a['token']);
		$response = $api->specialQuery ("AffiliateUser","uniqueEmail",array('email'=>$email));
		//print_r ($response);
		if ($response['response']["errorMessage"]) logError ("AffiliateUser.uniqueEmail",var_export($response,true));
		if ($response['response']['data'] != '1') $emailsExist++; //$email_status = -1;
	}
	if ($emailsExist == count($apis)) return -1;
	else return 1;
	//return $email_status;
}

if (isset($_GET['checkemail']) && $_GET['api'] == "roi") {
